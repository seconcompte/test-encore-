 ...
 *
 * Ce script intègre :
 * - Un serveur Express gérant le flux OAuth2 (/login, /callback, /collect)
 * - Un bot Discord (commandes textuelles et slash)
 * - Une connexion à une base de données PostgreSQL via le module "postgres"
 *
 * Assurez-vous que vos variables d'environnement sont injectées (via Koyeb, par exemple)
 * et que le fichier .env (si utilisé en local) ne soit pas poussé sur GitHub.
 ******************************************************************/
...
const ENV_PORT = process.env.PORT;           // Ex: 8000
...
// Pour les settings par défaut, nous utilisons ceux définis dans l'interface
...
// Création de la connexion PostgreSQL via le module 'postgres'
const sql = postgres({
...
// Recherche des comptes alternatifs d'un utilisateur par comparaison de stable_hash
...
// Utilisation d'un Set pour éviter le double traitement d'une même soumission
...
async function processSubmission(submission) {
  ...
  try {
    ...
    console.error("[ProcessSubmission] Erreur lors du décodage:", err.message);
    return;
  }
  ...
  // Ignorer certaines IP (exemple: agents Discord)
  ...
  // Vérifier si l'utilisateur existe déjà dans user_data
  const rows = await sql`SELECT * FROM user_data WHERE user_id=${userId} AND guild_id=${guildId}`;
  if (rows.length > 0) {
    const row = rows[0];
    if (row.email !== null) {
      console.log(`[ProcessSubmission] ${userId} est déjà vérifié en mode haute.`);
      return;
    } else {
      if (submission.mode === "basic") {
        console.log(`[ProcessSubmission] ${userId} a déjà une vérification basique – nouvelle basique refusée.`);
        return;
      } else if (submission.mode === "high") {
        // Conversion de la vérification basique en haute : fusionner les infos
        let oldHashes;
        try {
...
// GET /login : redirige l'utilisateur vers l'URL OAuth2 de Discord (mode haute uniquement)
...
// GET /callback : échange le code OAuth2 contre un token, récupère l'e-mail et la liste des guildes,
// stocke temporairement ces informations dans /tmp, puis redirige vers /collect.
app.get("/callback", async (req, res) => {
  ...
  try {
    ...
    if (mode === "high") {
      ...
      // Stocker temporairement les données dans /tmp avec un token temporaire
...
// GET /collect : Récupère, pour le mode high, les données stockées dans /tmp et traite la soumission.
...
// ---------------------- SLASH COMMANDS et Commandes textuelles ----------------------
...
// Démarrage d'Express sur le port spécifié
app.listen(PORT, () => {
